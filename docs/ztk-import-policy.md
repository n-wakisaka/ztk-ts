...
ここで重要なのは、semantic policy と runtime materialization scope を分けることです。

- semantic / normalize-semantic
  - upstream 想定の `ztk` / `stl` / `obj` / `ply` / `dae` を valid import format として受理し、`importName` / `importScale` を保持する
  - したがって `.dae` import も ZTK としては妥当な authoring 情報として扱う
- materialize-runtime
  - built-in loader 実装済み format だけを concrete geometry に解決する
  - `.dae` は upstream では optional build feature でもあるため、現時点では runtime materialization 未対応とする
...
- `.dae` は semantic policy では recognized format として保持するが、runtime materialization の built-in loader は未実装のままにする
...
- `unsupported-import-resolution`
  ...
  - 現状は `dae` がこれに該当する
  - これは「format unknown」ではなく、「semantic では受理するが runtime ではまだ解決できない」ことを意味する
...
5. `.dae` は upstream optional feature として、必要になるまで runtime materialization 実装を保留する

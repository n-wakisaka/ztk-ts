...
- `import` を持つ `zeo::shape`
  - diagnostic: `requires-external-shape-import`, `unsupported-import-format`, `unsupported-import-resolution`
  - effect: `runtime-materialization`
  - 意味: semantic model だけでは import 先の geometry 実体を保持していないため、format-aware materialized export は current built-in loader scope に依存して block されうる
  - semantic / normalize では upstream-known format を保持できるが、materialize-runtime では built-in loader scope 外の format が block されうる
  - つまり `mesh.dae` のような import は normalized save では保持できるが、runtime materialization は block される
...
- procedural polyhedron mirror
  ...
  - `ztk-ts` 側で import format を判定し、internal loader dispatch に渡す
  - `source.format` は upstream `zeo` 想定の `ztk/stl/obj/ply/dae` または `unknown`
  - built-in loader がない、または geometry がまだ materialize できない場合は serializer は `supported: false` を返す
  - diagnostics:
    - `materializes-shape-import` when built-in loader supplied concrete geometry
    - `requires-external-shape-import` when semantic model only has import reference
    - `unsupported-import-format` when the file suffix is outside upstream expectations
    - `unsupported-import-resolution` when the format is known but built-in loader is not implemented yet
    - `import-resolution-failed` when built-in loader attempted to load but failed
    - current built-in scope is `.ztk`, `.obj`, `.stl`, and `.ply`; other upstream-known formats still report `unsupported-import-resolution`
    - `.dae` is treated as semantically valid but runtime-unresolved
    - malformed or unreadable `.ztk` / `.obj` / `.stl` / `.ply` inputs report `import-resolution-failed`
  - effect: `runtime-materialization`
- imported transform bake scope
  - `.ztk` import では imported shape 側の `pos` / `att` / `rot` / `frame` を resolved geometry に bake する
  - `.obj` / `.stl` / `.ply` import では resolved polyhedron vertices に outer shape transform / importScale を適用する
  - procedural polyhedron の回転付き bake のように current authoring-preserving materialization で扱えないケースは `import-resolution-failed` とする
...
将来は `materialize-runtime` とは別に、upstream `FPrintZTK` により近い fully baked export を追加候補として扱う。

- 目的
  - runtime 連携用の参照解決だけでなく、最終 shape/body 表現まで upstream print 寄りに落としたい場合に使う
- 想定する挙動
  - `mirror` / `import` を保持せず resolved shape に展開する
  - procedural `polyhedron` の `loop` / `prism` / `pyramid` を必要に応じて `vert` / `face` に bake する
  - 必要なら `nurbs` も upstream runtime/export と整合する mesh/polyhedron 表現へ落とす
- 注意点
  - authoring 情報を大きく失う不可逆 export なので、`normalize-semantic` や現在の `materialize-runtime` とは分けて扱う
  - editor の通常 save には使わず、明示的な export API として隔離する

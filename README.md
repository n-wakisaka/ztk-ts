...
- `docs/ztk-import-policy.md`: `zeo::shape import` の format-aware materialization policy

## Examples

- `examples/api-basics.ts`: parse / resolve / preserve-source / normalize-semantic の流れを日本語コメント付きで示す
- `examples/materialized-runtime.ts`: materialized runtime analysis/export と import resolver の使い方を日本語コメント付きで示す
...
- `analyzeSemanticZtkMaterializedRuntime()` is a preflight helper for future runtime/export materialization; it reports which shapes would be materialized and whether the current semantic model has enough information to do it, including format-aware diagnostics for `import`
- `serializeSemanticZtkMaterializedRuntime()` currently supports mirror-based materialization for primitive shapes, NURBS, explicit polyhedra, and procedural polyhedra whose loop structure is preserved semantically; `import` is format-aware but built-in import loaders are not implemented yet
